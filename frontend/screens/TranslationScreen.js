...
export default function TranslationScreen() {
  ...
  // --- Voici les fonctions que j'avais enlevées par erreur ---

  const handleGoHome = () => { navigation.navigate('Video'); };
  const handleGoToHistory = () => { navigation.navigate('History'); };
  const handleGoToSettings = () => { navigation.navigate('Settings'); };
  const handleClose = () => { navigation.navigate('Video'); };

  const handleLogout = () => {
    setUserMenuVisible(false);
    // Ici, tu appelleras la fonction de déconnexion de ton AuthContext
    Alert.alert("Déconnexion", "Vous avez été déconnecté.");
    navigation.navigate('Login');
  };
