    ...
    const importVideo = async () => {
      try {
        const video = await DocumentPicker.getDocumentAsync({
          type: 'video/*',
          copyToCacheDirectory: true, // Recommandé pour la fiabilité
          multiple: false,
        });
        ...
        if (fileSizeInMB > MAX_FILE_SIZE_MB) {
          Alert.alert('Fichier trop volumineux',
            `La vidéo sélectionnée pèse ${fileSizeInMB} Mo. Veuillez sélectionner une vidéo de moins de ${MAX_FILE_SIZE_MB} Mo.`
          );
          return;
        }
 
        // C'est ici que vous appelez votre fonction backend ou de traitement
        // Assurez-vous que la fonction 'local_video' est bien définie ou importée dans ce fichier
        ...
        if (data.task_id) {
            ...
            Alert.alert('Erreur', data.message || "L'envoi a échoué, pas d'ID de tâche reçu.");
        }
 
      } catch (error) {
        console.error('Erreur lors de l\'importation de la vidéo :', error);
        Alert.alert('Erreur d\'importation', `Un problème est survenu : ${error.message}`);
      }
    };
        ...
        <View style={styles.container}>
            ...
            <View style={styles.controlsContainer}>
                <View style={styles.sideControls}>
                    <TouchableOpacity style={styles.controlButton} onPress={handleFlipCamera}><Icon name="flip-camera-ios" size={28} color="black" /></TouchableOpacity>
                    <TouchableOpacity style={styles.controlButton} onPress={handleTimerPress}><Icon name="timer" size={28} color="black" /></TouchableOpacity>
                </View>
                ...
                <View style={styles.sideControls}>
                    <TouchableOpacity style={styles.controlButton} onPress={importVideo}><Icon name="file-upload" size={28} color="black" /></TouchableOpacity>
                    <TouchableOpacity style={styles.controlButton} onPress={handleInfoPress}><Icon name="info-outline" size={28} color="black" /></TouchableOpacity>
                </View>
            </View>
        </View>
    );
